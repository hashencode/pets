        ...
        <DefaultLayout head="weight" safeAreaViewConfig={{ edges: ['top', 'left', 'right'] }}>
            ...
            <View className="px-3 py-4">
                ...
                <View className="flex flex-row flex-wrap justify-around gap-3">
                    {[
                        { label: 'vomit', icon: '', key: 'vomit' }, // 呕吐
                        { label: 'weight', icon: '', key: 'weight' }, // 称重
                        { label: 'appetite', icon: '', key: 'appetite' }, // 食欲
                        { label: 'hospital', icon: '', key: 'hospital' }, // 医院
                        { label: 'camera', icon: '', key: 'camera' }, // 摄像头
                        { label: 'camera', icon: '', key: 'camera2' }, // 摄像头
                        { label: 'camera', icon: '', key: 'camera3' }, // 摄像头
                        { label: 'camera', icon: '', key: 'camera4' }, // 摄像头
                    ].map(({ label, icon, key }) => {
                        return (
                            <View className="w-20" key={key}>
                                <Image
