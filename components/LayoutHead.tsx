    ...
    return (
        <View className={classNames('flex h-14 w-full flex-row items-center justify-between bg-white', styles?.root)}>
            ...
            <View className={classNames('z-10 flex w-full flex-col items-center justify-center gap-x-1 px-14', styles?.body)}>
                <Text className={classNames('line-clamp-1 w-full text-center text-xl font-medium', styles?.title)}>{title}</Text>
                {subtitle ? (
                    <Text className={classNames('line-clamp-1 w-full text-center text-sm text-secondary', styles?.subtitle)}>
                        {subtitle}
                    </Text>
                ) : null}
            </View>
